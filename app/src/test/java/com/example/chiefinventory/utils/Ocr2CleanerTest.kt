...
class Ocr2CleanerTest {
    ...
    fun `clean with empty input list`() {
        ...
        val result = Ocr2Cleaner.clean(emptyList(), mockResources)
    ...
    fun `clean handles whitespace only lines`() {
        ...
        val input = listOf(" ", "\t", "\n", "  \t  ")
        val result = Ocr2Cleaner.clean(input, mockResources)
        assertTrue(result.isEmpty())
    }
    ...
    fun `isTechnicalDimension valid  x  connector`() {
        ...
        assertTrue(Ocr2Cleaner.isTechnicalDimension("5 mm x 5 mm"))
        assertTrue(Ocr2Cleaner.isTechnicalDimension("10x10cm"))
    }
    ...
    fun `isTechnicalDimension valid  sur  connector`() {
        ...
        assertTrue(Ocr2Cleaner.isTechnicalDimension("5 mm sur 5 mm"))
    }
    ...
    fun `isTechnicalDimension case insensitive connectors`() {
        ...
        assertTrue(Ocr2Cleaner.isTechnicalDimension("5 MM SUR 5 MM"))
        assertTrue(Ocr2Cleaner.isTechnicalDimension("10 X 10 CM"))
    }

    @Test
    fun `isTechnicalDimension area units detection`() {
        // Ensure units like 'mm2' or 'cm2' (e.g., '100 cm2 x 50 cm2') trigger a positive match.
        assertTrue(Ocr2Cleaner.isTechnicalDimension("100 cm2 x 50 cm2"))
        assertTrue(Ocr2Cleaner.isTechnicalDimension("10 mm2 sur 10 mm2"))
    }
    ...
    fun `isTechnicalDimension dimension without connector`() {
        ...
        assertFalse(Ocr2Cleaner.isTechnicalDimension("5 mm"))
        assertFalse(Ocr2Cleaner.isTechnicalDimension("250 g"))
    }
    ...
    fun `isTechnicalDimension connector without dimension unit`() {
        ...
        assertFalse(Ocr2Cleaner.isTechnicalDimension("5 kg x 5 kg"))
    }
    ...
    fun `isTechnicalDimension boundary match check`() {
        ...
        assertFalse(Ocr2Cleaner.isTechnicalDimension("100 mms x 100 mms"))
        assertTrue(Ocr2Cleaner.isTechnicalDimension("100 mm x 100 mm"))
    }
}
